...
def mainWindow() -> None:
    ...
    root.bind(f'<{MODIFIER}-o>', load)
    root.bind(f'<{MODIFIER}-s>', save)
    root.bind(f'{MODIFIER}-z', undo)
    root.bind(f'<{MODIFIER}-Shift-KeyPress-s>', lambda: save(saveas = True))
    root.protocol('WM_DELETE_WINDOW', close)
    root.bind(f'<{MODIFIER}-f>', lambda e: findWindow(listbox))
